...
#Previosly generated key is used
key=b'\xa4\x8f\xbd\xdd\x83+\xeb\xf2\x9c\x1d0\x87\xb9\xb9P\xbe\x0b8\xfc\xf4\xa0\t~#\xbb\xaf\xa2\xb1\xd25\xdc\x8d'
cipher = AES.new(key, AES.MODE_ECB)

# AES only takes data with a length of 16
# Gets the length of our message and adds the appropriate amount of curly brackets
# to make it evenly divisible by 16
def pad(s):
    return s + ((16 - (len(s) % 16)) * '$')

def encrypt(plaintext):
    global cipher
    AES_msg=cipher.encrypt(pad(plaintext).encode('utf-8'))
    return base64.b64encode(AES_msg).decode('utf-8') #returns String
...
def show_data(img):  
    ...
    for bytes in allBytes:  
        ...
        if chr(int(bytes, 2)) == '~' and chr(int(allBytes[allBytes.index(bytes) + 1], 2)) == '~' and chr(int(allBytes[allBytes.index(bytes) + 2], 2)) == '~':  
            break  
